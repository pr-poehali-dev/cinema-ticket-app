...
export default function Index() {
  ...
  const [currentPage, setCurrentPage] = useState<'home' | 'movies' | 'showtimes' | 'profile'>('home');
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <nav className="flex items-center gap-6">
              <button onClick={() => setCurrentPage('movies')} className={`transition-colors flex items-center gap-2 ${currentPage === 'movies' ? 'text-primary' : 'text-foreground/80 hover:text-foreground'}`}>
                <Icon name="Film" size={20} />
                <span className="hidden sm:inline">Фильмы</span>
              </button>
              <button onClick={() => setCurrentPage('showtimes')} className={`transition-colors flex items-center gap-2 ${currentPage === 'showtimes' ? 'text-primary' : 'text-foreground/80 hover:text-foreground'}`}>
                <Icon name="Calendar" size={20} />
